...
def add_player_to_tournament(player_id, tour_id):
    try:
        ...
        # add_player_2_tournament_blockchain(tournament.name, player)
...
class RoomsConsumer(AsyncWebsocketConsumer):
    ...
    async def update_match_result(self, data):
        ...
        if not match.end_time:
            ...
            # print('on send un match a la blockchain')
            # add_match_to_tournament_blockchain(tournament.name , match)
            # print('cest bon !')
    ...
    async def handle_tour_matches(self, data):
        ...
        if tournament.terminated == True:
            # add_winner_to_tournament_blockchain(tournament.name, tournament.winner.login)
            await self.send(text_data=json.dumps({
